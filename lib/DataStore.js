...
class DataStore {
	...
	//      this._utils.debugLog("unload called");
	//      this.storeCommands();
	//   });
	//}
	...
	storeCommands() {
		...
		fs.writeFile(fileName, data,
			function (err) {
				if (err) {
					//this._utils.debugLog("**> Storing dataArray failed: " + err);
	...
	readCommands(callback) {
		...
		fs.access(fileName, fs.constants.F_OK | fs.constants.W_OK | fs.constants.R_OK, (err) => {
			if (err) {
				//this._utils.debugLog('**> file does not exist or no access: ' + err);
					...
					function (err, data) {
						if (err) {
							...
							try {
								...
								//this._utils.debugLog('**> parse failed: ' + err);
	...
	deleteAllCommands() {
		...
		/*
				...
				fs.unlink( fileName, (err)  => {
					if(err) {
						this._utils.debugLog('**> Cannot remove file ' + fileName + ', err = ' + err );
					}
				})
		*/
	}
	...
	deleteCommand(cmdName) {
		//this._utils.debugLog('datastore.deleteCommand '+cmdName)
	...
	renameCommand(oldName, newName) {
		//this._utils.debugLog('datastore.renameCommand  old='+oldName + '  new='+newName)
