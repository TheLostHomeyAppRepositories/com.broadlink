...
const DeviceInfo = require('./DeviceInfo.js').default;
...
class BroadlinkDriver extends Homey.Driver {
	...
	onPair(session) {
				...
				.then(localAddress => {
					...
					this._communicate.discover(5, localAddress, data.address)
						.then(info => {
							...
							var devinfo = this._utils.getDeviceInfo(info.devtype, this.CompatibilityID, this.homey);
		...
		session.setHandler('list_devices', async () => {
			this._utils.debugLog("==>Broadlink - list_devices: " + JSON.stringify(this.discoveredDevice.device));
